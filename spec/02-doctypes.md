...
| `login_url`     | Small Text             |      | Y         | The one-click sign-in URL minted after boot (the bench-cli admin session verb in admin mode, `bench browse` in site mode); empty when the golden's bench-cli carries no admin session verb. Short-lived — see `login_url_expires_at`; regenerated on demand via `regenerate_login_url()`. `no_copy`. Surfaced to Central once the pilot is Running. Controller-written. |
