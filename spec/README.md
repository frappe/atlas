...
| Drive a host through Boat       | `Server` → tick **`boat_enabled`** (Atlas drives that host through its resident `boat` daemon over the management tunnel instead of per-Task SSH; every lifecycle button is unchanged, and clearing the flag returns the host to the SSH path). *Lands with WO-0 — [33-boat.md § delivery order](./33-boat.md)* | [33-boat.md](./33-boat.md) |
| Roll a host's Boat version      | `Server` → set **`boat_version`**; Atlas pushes it (canary first, then waves), observes the running version in the host's export, and the host rolls back to N-1 on a failed post-swap health check. *Lands with WO-5b* | [33-boat.md](./33-boat.md) |
