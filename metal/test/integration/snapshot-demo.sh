...
IMAGES_DIR=${METALD_IMAGES_DIR:-$WORKDIR/images}
...
# Point metald at the dev workdir and force the listen address to the one we poll,
# so nothing in the environment can send it elsewhere.
export METALD_WORKDIR="$WORKDIR"
export METALD_CHROOT_BASE="${METALD_CHROOT_BASE:-$WORKDIR/chroot}"
export METALD_VAR_DIR="${METALD_VAR_DIR:-$WORKDIR/vms}"
export METALD_KERNEL_DIR="${METALD_KERNEL_DIR:-$WORKDIR/kernels}"
export METALD_IMAGES_DIR="$IMAGES_DIR"
export METALD_JAILER="${METALD_JAILER:-$WORKDIR/bin/jailer}"
export METALD_FIRECRACKER="${METALD_FIRECRACKER:-$WORKDIR/bin/firecracker}"
export METALD_LISTEN="$ADDR"
LOG=/tmp/metald-demo.log

# Start metald if the API is not already answering. Use `serve` when the host is
# already bootstrapped (the pool exists); otherwise `up` bootstraps it first.
if ! api /health >/dev/null 2>&1; then
	...
	sub=up; zpool list "$POOL" >/dev/null 2>&1 && sub=serve
	echo "==> starting metald ($sub; log: $LOG)"
	(cd "$MODULE" && go build -o "$WORKDIR/bin/metald" ./cmd/metald)
	"$WORKDIR/bin/metald" "$sub" >"$LOG" 2>&1 &
	METALD_PID=$!
