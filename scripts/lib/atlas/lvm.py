...
class PoolUsage:
    ...
    def too_full_to_snapshot(self) -> bool:
        ...
        return (
            self.data_percent >= self.FULL_THRESHOLD
            or self.metadata_percent >= self.FULL_THRESHOLD
        )
...
class LogicalVolume:
    ...
    def is_protected(self) -> bool:
        return self.name == self._pool.pool_name or self.name.startswith(
    ...
    def _wait_for_node(self) -> None:
        ...
        if not self._node_is_block_device():
            raise RuntimeError(
                f"LV {self.name} activated but {self.device_path} is not a block device"
            )
...
class ThinPool:
    ...
    def ensure(self) -> None:
        ...
        if not self._pool_lv.exists:
            run("sudo", "lvcreate", "--type", "thin-pool", "--name", self.pool_name,
                "--poolmetadatasize", self.metadata_size, "--extents", "100%FREE",
                self.volume_group, quiet=True)
        run("sudo", "vgchange", "-ay", "-K", self.volume_group, quiet=True)
    ...
    def import_base_image(self, source_file: str, image_name: str, disk_gigabytes: int) -> LogicalVolume:
        ...
        if lv.exists:
            return lv
        run("sudo", "lvcreate", "--type", "thin", "--thinpool", self.pool_name,
            "-V", f"{disk_gigabytes}G", "-n", lv.name, self.volume_group, quiet=True)
        lv.activate()
        try:
            run("sudo", "dd", f"if={source_file}", f"of={lv.device_path}",
                "bs=4M", "conv=fsync", "status=none")
        except Exception:
            run("sudo", "lvremove", "-f", f"{self.volume_group}/{lv.name}",
