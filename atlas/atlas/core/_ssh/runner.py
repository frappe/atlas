...
def run_task(
	*,
	...
	if server is not None and is_fake_server(server):
		from atlas.atlas.core.providers.fake_tasks import run_fake_task
	...
	if server is not None and scripts_catalog.runs_on_boat_http(script):
		from atlas.atlas.core.boat_client import run_boat_host_task
...
def run_probe(
	*,
	...
	if is_fake_server(server):
		from atlas.atlas.core.providers.fake_tasks import fake_stdout
	...
	if scripts_catalog.runs_on_boat_http_read(script):
		from atlas.atlas.core.boat_client import run_boat_host_read
...
def execute_task(task_name: str) -> None:
	...
	if is_fake_server(task.server):
		from atlas.atlas.core.providers.fake_tasks import finalize_fake_task
...
def connection_for_server(server) -> Connection:
	...
	import atlas
	from atlas.atlas.core.secrets import get_ssh_key_from_disk
