...
from atlas.atlas.core.server_providers.base import ACCEPTED_OS_VERSIONS, ImageInfo, SizeInfo
...
class ScalewayCatalog:
	...
	def get_server_sizes(self, offers: object) -> tuple[SizeInfo, ...]:
	...
	def get_server_images(self, images: object) -> tuple[ImageInfo, ...]:
	...
	def get_offer_id(self, size_document: object, subscription_period: str) -> str:
		...
		if not isinstance(offer_id, str):
			raise ScalewayError(
				f"Server Size {size_document.name} has no {subscription_period} Scaleway offer ID"
			)
		return offer_id
	...
	def get_private_network_option_id(self, size_document: object, subscription_period: str) -> str:
		...
		for option in self._offer(size_document, subscription_period).get("options", []):
			if (
				...
				return option["id"]
		raise ScalewayError(f"Server Size {size_document.name} has no Private Network option")

	def _merge_offer(self, size: SizeInfo | None, offer: Mapping) -> SizeInfo:
