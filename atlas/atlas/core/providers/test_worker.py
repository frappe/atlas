...
from atlas.atlas.core.providers.base import ProvisionResult, ServerNetworking
...
class TestReconcilePendingServers(IntegrationTestCase):
	...
	def test_reconciler_is_registered_in_scheduler(self) -> None:
		...
		self.assertIn("atlas.atlas.core.providers.worker.reconcile_pending_servers", cron_jobs)
...
class TestCheckNetworkdLiveness(IntegrationTestCase):
	...
	def test_probe_is_read_only(self) -> None:
		...
		with (
			patch("atlas.atlas.core.ssh.connection_for_server", return_value=MagicMock()),
			patch("atlas.atlas.core.ssh.ssh_key_file") as key_file,
			patch("atlas.atlas.core.ssh.run_ssh", side_effect=_fake_run_ssh),
		):
	...
	def test_backstop_is_registered_in_scheduler(self) -> None:
		...
		cron_jobs = [job for jobs in hooks.scheduler_events.get("cron", {}).values() for job in jobs]
		self.assertIn("atlas.atlas.core.providers.worker.check_networkd_liveness", cron_jobs)
