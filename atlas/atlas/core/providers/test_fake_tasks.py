...
from atlas.atlas.core.providers.fake_tasks import (
	_fake_stdout,
	_parse_script_list,
	is_fake_server,
	run_fake_task,
...
class TestRunFakeTask(_FakeServerCase):
	def test_run_task_succeeds_without_ssh(self) -> None:
		...
		with patch("atlas.atlas.core._ssh.transport.subprocess.run") as subprocess_run:
			task = run_task(
...
class TestVirtualMachineThroughFake(_FakeServerCase):
	...
	def test_provision_then_terminate(self) -> None:
		...
		with (
			patch("atlas.atlas.core._ssh.transport.subprocess.run") as subprocess_run,
