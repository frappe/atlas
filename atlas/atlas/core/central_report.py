...
from atlas.atlas.core.central import CentralError
...
def on_vm_update(doc, method=None):
	...
	if _status_changed(doc):
		...
		# event after the in-guest mint. Core is PaaS-blind: services owns the
		# front-door check (`vm.status_suppressed`); a plain VM is never suppressed and
		...
		if not callbacks.run_first("vm.status_suppressed", doc.name):
...
def _vm_payload(doc) -> dict:
	...
	# the VM). Those front-door fields — gateway_url, and once Running the login_url +
	# its expiry, plus the aggregate's status override — are folded in by the
	# `vm.payload_augment` callback (services), so core builds only the plain-VM shape
	# and stays PaaS-blind. A plain VM (proxy, operator machine) keeps the None defaults.
	...
	callbacks.run("vm.payload_augment", payload, doc.name)
	return payload
