...
def after_insert(doc) -> None:
	...
	frappe.enqueue(
		...
		site_name=doc.name,
		# The bench enrollment inputs ride the job to the bench's bench.toml. Flags are set by
		# create_vm. (The pilot credential id is stamped on the VM at insert, not here.)
...
def _provision_backing_vm(doc) -> str:
	...
	vm_doc = {
		...
		"correlation_id": doc.flags.get("correlation_id"),
	}
