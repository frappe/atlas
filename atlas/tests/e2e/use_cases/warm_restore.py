...
def _resolve_or_bake_warm(server, base_image: str) -> "frappe.model.document.Document":
	...
	with patch.object(ImageBuild, "after_insert", lambda self: None):
		build = frappe.get_doc(
			{
				"doctype": "Image Build",
				# All site variants are warm (warm_entrypoint=warm.sh); bench-v16 is the
				# one that also registers_as=default_bench_snapshot — the warm self-serve
				# golden this e2e asserts below. The `recipe` Select no longer accepts the bare
