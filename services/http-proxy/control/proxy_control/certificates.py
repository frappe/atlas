...
class CertificateStore:
	...
	def _save_state(self, region: str) -> _State:
		...
		links = {
			name: os.readlink(self.directory / name) if (self.directory / name).is_symlink() else None
			for name in ("fullchain.pem", "privkey.pem")
		}
		files = {name: self._read_file(target / name) for name in ("fullchain.pem", "privkey.pem")}
		return _State(
