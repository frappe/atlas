...
def test_openresty_is_apt_package():
	...
	assert res.returncode == 0, f"OpenResty binary not dpkg-owned (built from source?): {res.stderr}"
...
def test_running_binary_matches_the_build_pin(nginx_V):
	...
	assert running == pin, f"running OpenResty {running} != setup.sh pin {pin} (rebake needed?)"
...
def test_no_compiler_toolchain_in_the_image():
	...
	assert not res.stdout.strip(), f"a compiler is installed, but the build compiles nothing: {res.stdout!r}"
...
def test_luajit_is_the_openresty_fork():
	...
	res = exec_proxy("sh", "-c", "/usr/local/openresty/luajit/bin/luajit -v", check=False)
	if res.returncode != 0:
...
def test_package_config_untouched_and_unused():
	...
	"""
	pkg_conf = exec_proxy("cat", "/usr/local/openresty/nginx/conf/nginx.conf", check=False)
...
def test_worker_can_persist_map_to_disk():
	...
	assert '"dumped":true' in dump.stdout.replace(" ", ""), f"POST /v1/dump failed: {dump.stdout!r}"
