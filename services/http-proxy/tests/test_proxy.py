...
def test_v1_patch_and_delete_update_one_site():
	...
	status, body = admin("PATCH", "/v1/sites/targeted", json.dumps({"address": VM_A}))
	assert status == 200 and json.loads(body)["address"] == VM_A
	assert json.loads(admin("GET", "/v1/healthz")[1])["entries"] == before + 1
	assert fetch("targeted")[0] == 200

	status, body = admin("PATCH", "/v1/sites/targeted", json.dumps({"address": VM_B}))
...
def test_plain_http_proxies_to_the_vm():
	...
	assert "location:" not in headers.lower(), f"port 80 redirected instead of forwarding: {headers!r}"
...
def noregion_set_site(key: str, address: str) -> tuple[int, str]:
	return noregion_admin("PATCH", f"/v1/sites/{key}", json.dumps({"address": address}))
