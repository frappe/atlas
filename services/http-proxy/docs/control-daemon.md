...
The control daemon is the main API for the controller. It listens on HTTP at `0.0.0.0:9000` and `[::]:9000` by default.
...
Use this API only on a trusted controller or WireGuard network. The bearer password is sent over HTTP and can be replayed by anyone who can observe the connection.
